...
Waveform research checkpoint: actual RX3 PWAV/PWV2 writers generated six preview sections whose 1,012 bytes survived the current BiteDJ parser exactly. Both scalar and NEON preview packers match an independent five-bit/three-bit rule over all 65,536 byte pairs. Actual three-band display conversion now matches an independently calculated reference in 790 cases plus three guard cases. The third lane uses a sine-squared transfer and a shared maximum-band-sum cap; BiteDJ intentionally uses a different display normalization. Caller encoding, thinning and rendering must be traced before using this to change BiteDJ. These are isolated execution results, not full app or audio-generation validation. No vendor implementation or assets are included.
...
Database/browser research checkpoint: executed all 74 discovered table creators and captured 391 column declarations, plus 36 index creators whose table/column names match that inventory. These describe runtime declarations, not PDB bytes or index uniqueness. Actual playlist selection passes 512 mode-1 flag cases and 49 parent-ID comparisons with EDB providers intercepted. Actual playlist browse conversion passes 1,805 controlled cases: it filters signed sequence values above 1,000 (not a demonstrated row-count limit), preserves IDs, converts an attribute byte into an insertion type and clears on cancellation. NAME is not read there; label resolution remains open. BiteDJ should retain arbitrary valid ordering rather than copy this firmware cutoff. No new production change is justified solely by these results; no firmware implementation or assets are included.

Waveform motion comparison: actual stock RX3 position initialization quantizes to whole pixels; increasing synthetic update input from60 to120Hz retains37.5 position changes/sec at factor4. BiteDJ also rounds shared display position, and multiple shader renderers round their sample origins. Its actual pre-render calculation, executed through the existing time-scale fixture, changes position on240/240 simulated60Hz updates at default zoom3 but110/240 at zoom16. Native441Hz and imported150Hz analysis behave identically. This is display-scale/rounding evidence, not a GPU benchmark or a fixed FPS cap. A future fractional-scrolling change must coordinate waveform and cue/grid/phrase overlays; no speculative production rendering patch is included.
