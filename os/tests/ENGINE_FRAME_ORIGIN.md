...
## Default and adjusted main-cue source state
...
The native reset behavior is now also executed under PC ARM emulation using the real constructor at 0x182b4c0 and verified vtable. Setting the flag at object+0x38 alone leaves the main double unchanged. Reset at 0x15f4100 copies object+0x30 to object+0x28 and clears that flag. Synthetic secondary values 12345.5 and 0 both survive exactly, starting from main values 45678.25 and -1 respectively. The executable hash is unchanged after the run. See `tools/engine-reader/diagnostics/check-cue-reset.py` and its recorded results for reproduction. This confirms object reset semantics, not database-field correspondence, a database zero sentinel, or full source-library loading. Cue import remains deferred.
