...
The Library menu opens **Preview Engine DJ library…**. Choose a drive containing `Engine Library/Database2/m.db`, or the Engine Library folder itself. The asynchronous reader shows track titles/artists, lengths, populated hot-cue/loop counts and independently resolved media availability. Reading can be cancelled; rejecting or closing the dialog cancels the request too. Large libraries use a table model rather than allocating a widget for every track. Rows/buttons have 44-pixel minimum touch targets and the table supports touch scrolling.

Preview alone is read-only. The separate **Import metadata + playlists** action applies supported metadata and playlists; see [ENGINE_APPLY.md](ENGINE_APPLY.md) for partial-save, conflict and cancellation behavior and deferred fields. BiteDJ replaces helper-supplied media paths and context with its own resolution from the selected canonical folders, on the reader worker. Media availability remains a filesystem observation, not permission to open a file later or proof of audio identity/decode compatibility.
