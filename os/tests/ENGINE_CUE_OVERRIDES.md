...
Valid negative cue positions now round-trip in version2. The previous writer serialized these positions, but the reader rejected the whole snapshot, including positive cues. Version2 now distinguishes an absent end field from a valid negative end; this also preserves loops entirely before zero and loops crossing zero. Legacy version1 negative-position/sentinel handling is unchanged.

The regression failed against the prior completed application archive with “negative start rejected complete snapshot”. After the fix, the full application build, completed-archive codec probe and private-namespace actual SQLite save/fresh reload passed. Coverage includes a negative hot cue with unset end, negative main cue, wholly negative loop, cross-zero loop, and an end at exactly minus one second. This verifies persistence, not deck playback before zero.
