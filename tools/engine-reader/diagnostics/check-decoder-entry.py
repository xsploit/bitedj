#!/usr/bin/env python3
"""Inspect native Engine reader addresses under PC ARM emulation, without main.
...
source=Path(__file__).with_name('decoder-entry-probe.c').resolve()
with tempfile.TemporaryDirectory(prefix='engine-decoder-entry-') as temporary:
 ...
 result={'scope':'Native ELF relocation and RTTI only; no Engine main/decoder/Pi execution',
  ...
  'passed':run.returncode==0 and 'ENTRY_PROBE PASS' in run.stdout}
