...
int main(int argc,char**argv){
 if(argc!=3)return 2;
 std::ifstream f(argv[1],std::ios::binary);kaitai::kstream stream(&f);rekordbox_anlz_t a(&stream);
...
'''
 ...
 count=0
 for comment in ['', 'Café 🎧']:
  text=(comment+'\0').encode('utf-16-be')
  for tail in range(5):
   entry=b'PCP2'+struct.pack('>III',16,44+len(text)+tail,1)+struct.pack('>BBHII',1,0,1000,1234,0)+bytes(8)+struct.pack('>HHI',0,0,len(text))+text+bytes([9,20,40,60])[:tail]
 ...
 print(f'{count} production parser/helper cases passed (empty/Unicode comments, 0–4 color bytes)')
